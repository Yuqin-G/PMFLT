...
class Classification:
    ...
    def __init__(self, cfg):
        ...
        self._y_true = []
        self._y_pred = []
        if cfg.EXPERIMENT.PER_CLASS_RESULT:
    ...
    def evaluate(self):
        ...
        # print("=> Correct/Total: ({}/{})".format(self._correct, self._total))
        # print("=> Accuracy: {:.3f}% ({}/{})".format(acc, self._correct, self._total))
        ...
        if self.cfg.EXPERIMENT.COMPUTE_CMAT:
            cmat = confusion_matrix(
                self._y_true, self._y_pred, normalize="true"
            )
            save_path = os.path.join(os.path.join(self.cfg.EXPERIMENT.OUTPUT_DIR, self.cfg.DATASET.NAME_SPACE[0]+'_'+str(self.cfg.DATASET.NUM_SHOTS)),
                                     "cmat.pt")
...
def compute_accuracy(output, target, topk=(1, )):
    ...
    #     print(target)
    #     res = torch.cat([pred.view(-1, 1), target.view(-1, 1)], dim=1)
    #     print("res: ", res.shape)
    #     res = res.cpu().numpy()
