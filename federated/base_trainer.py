...
class TrainerBase:
    ...
    def set_model_mode(self, mode="train", names=None):
        ...
        for name in names:
            if mode == "train":
                ...
                self._models[name].training = True
                self.model.training = True
                self.model = self.model.to(self.device)
            elif mode in ["test", "eval"]:
                self._models[name].eval()
                self._models[name].training = False
    ...
    def init_writer(self, log_dir):
        if not self._swanlab_initialized:
            print(f"Initialize SwanLab logging")
            self._swanlab_initialized = True

    def close_writer(self):
        # SwanLab doesn't need explicit closing
        pass

    def write_scalar(self, data, global_step=None):
        if self._swanlab_initialized:
            swanlab.log(data, step=global_step)
    
    def log_scalar(self, data, global_step=None):
        if self._swanlab_initialized:
            swanlab.log(data, step=global_step)
    ...
    def train(self, start_epoch, max_epoch, epoch_iter=True):
        ...
        self.comm_round = max_epoch

        self.before_train()
        for self.epoch in range(self.start_epoch, self.comm_round):
            self.before_epoch()
            if epoch_iter:
                self.run_epoch()
            else:
