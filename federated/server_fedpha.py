...
from model.FedPHA import FedPHA, load_clip_to_cpu
from federated.utils import *
from federated.client_fedpha import Client_FedPHA


class Server_FedPHA(ServerBase):

    def create_client_instance(self, cfg, client_id, dataname, available_cls, available_data, dataset_info):
        return Client_FedPHA(cfg, client_id, dataname, available_cls, available_data=available_data, dataset_info=dataset_info)
    ...
    def build_model(self):
        ...
        self.model = FedPHA(cfg, global_classnames, clip_model, device = self.device)
        self.model.lambda_orthogonal = cfg.TRAINER.GL_SVDMSE.lambda_orthogonal
        self.model.alpha = cfg.TRAINER.GL_SVDMSE.alpha
        self.w = cfg.MODEL.W
    ...
    def train(self):
        self.before_train()
        self.global_model = copy.deepcopy(self.model.prompt_learner.ctx_global)
        ...
        for round in range(self.start_round, self.comm_round):
            ...
            for idx in idxs_users:
                ...
                w_local0 = copy.deepcopy(w_local['prompt_learner.ctx_global'])
                w_local1 = copy.deepcopy(w_local['prompt_learner.ctx_local'])
            ...
            self.model.prompt_learner.ctx_global.data.copy_(self.global_model.data)
