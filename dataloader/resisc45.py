...
class RESISC45Dataset(VisionClassificationDataset):
    ...
    * https://doi.org/10.1109/jproc.2017.2675998
    ...
    def __init__(
        ...
        valid_fns = set()
        with open(os.path.join(root, "resisc45", f"resisc45-{split}.txt")) as f:
            for fn in f:
...
# from dataloader.dm_federated import DatasetWrapper
...
class RESISC45:
    def __init__(self, cfg, available_classes=None, global_classes=None, relabel=True):
        self.data_name = "resisc45"
        location = os.path.abspath(os.path.expanduser(cfg.DATASET.ROOT))
        # 读取train和test split
        train_dataset = RESISC45Dataset(root=location, split='train')
        test_dataset = RESISC45Dataset(root=location, split='test')
        ...
        for idx in range(len(train_dataset)):
            ...
            # train.append(Datum(impath=impath, label=label, classname=' '.join(cname.split('_'))))
        ...
        for idx in range(len(test_dataset)):
            ...
            # test.append(Datum(impath=impath, label=label, classname=' '.join(cname.split('_'))))
            test.append(Datum(impath=impath, label=label, classname=cname))

        if available_classes is not None:
            # print("previous train: ", train)
