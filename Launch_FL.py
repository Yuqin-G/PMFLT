...
def main(args):
    ...
    for seed in seeds:
        ...
        cfg.SEED = seed
        cfg.freeze()
        setup_logger(os.path.join(cfg.OUTPUT_DIR, cfg.EXP_NAME, cfg.MODEL.NAME, str(cfg.FEDERATED.NUM_CLASS_PER_CLIENT)+"_"+str(cfg.DATASET.NUM_SHOTS), str(cfg.SEED)))
        ...
        if not args.no_train:
            if (cfg.MODEL.NAME == 'fedpgp'):
                result = fl_server.train_fedpgp()
            elif (cfg.MODEL.NAME == 'fedotp'):
                result = fl_server.train_fedotp()
            elif (cfg.MODEL.NAME == 'promptfl'):
                result = fl_server.train_promptfl()
            elif (cfg.MODEL.NAME == 'promptfolio'):
                result = fl_server.train_promptfolio()
            elif (cfg.MODEL.NAME == 'fedclip'):
                result = fl_server.train_fedclip()
            elif (cfg.MODEL.NAME == 'cbm'):
                # fl_server.train_cbm()
                result = fl_server.train_cbm1()
            elif (cfg.MODEL.NAME == 'pfedmoap'):
                result = fl_server.train_pfedmoap()
            elif (cfg.MODEL.NAME == 'fedpha'):
                result = fl_server.train_fedpha()
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", type=str, default="/home/guanzenghao/data", help="path to dataset")
    ...
    parser.add_argument(
        "--num_shots", type=int, default=64, help="number of samples each class"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--seeds", type=int, nargs="+", default=[43], help="list of seeds, e.g. --seeds 0 1 2 3"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--data_mode", type=str, default="few_shot", help="few_shot, dirichlet"
    )
    parser.add_argument(
        "--alpha", type=float, default=0.5, help="alpha"
    )
    parser.add_argument(
        "--num_clients", type=int, default=10, help="num_clients"
    )
