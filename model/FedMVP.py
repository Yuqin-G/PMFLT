...
class FedMVP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, device='cuda'):
        ...
        self.require_classnames = True
    
    
    def forward(self, image, classnames, dataname):
        root = "./data"

        if dataname == 'caltech101' or dataname == 'Caltech101':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/caltech101.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/caltech101.json'), classnames, dataname)     
                               
        if dataname == 'oxford_flowers' or dataname == 'OxfordFlowers':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/oxford_flowers.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/oxford_flowers.json'), classnames, dataname)
            
        if dataname == 'fgvc_aircraft' or dataname == 'FGVCAircraft':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/fgvc_aircraft.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/fgvc_aircraft.json'), classnames, dataname)
            
        if dataname == 'ucf101' or dataname == 'UCF101':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/ucf101.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/ucf101.json'), classnames, dataname)
                                            
        if dataname == 'oxford_pets' or dataname == 'OxfordPets':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/oxford_pets.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/oxford_pets.json'), classnames, dataname)
                                           
        if dataname == 'food101' or dataname == 'Food101':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/food101.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/food101.json'), classnames, dataname)
            
        if dataname == 'dtd' or dataname == 'DescribableTextures':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/dtd.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/dtd.json'), classnames, dataname)
            
        if dataname =='stanford_cars' or dataname == 'StanfordCars':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/stanford_cars.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/stanford_cars.json'), classnames, dataname)
                                      
        if dataname == 'sun397' or dataname == 'SUN397':
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/sun397.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/sun397.json'), classnames, dataname)
        
        if dataname in ['PACS_artpainting', 'PACS_cartoon', 'PACS_photo', 'PACS_sketch']:
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/pacs.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/pacs.json'), classnames, dataname) 
            
        if dataname in ['OfficeHome_art', 'OfficeHome_clipart', 'OfficeHome_product', 'OfficeHome_realworld']:
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/officehome.json'), classnames, dataname)
            descriptions = load_descriptions(os.path.join(root, 'attributes/officehome.json'), classnames, dataname)
        
        if dataname in ['VLCS_CALTECH', 'VLCS_LABELME', 'VLCS_PASCAL', 'VLCS_SUN']:
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/vlcs.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/vlcs.json'), classnames, dataname)
        
        if dataname in ['TerraIncognita_L38', 'TerraIncognita_L43', 'TerraIncognita_L46', 'TerraIncognita_L100']:
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/terraincognita.json'), classnames, dataname)
            descriptions = load_descriptions(os.path.join(root, 'attributes/terraincognita.json'), classnames, dataname)
        ...
        if dataname in ['DomainNet_clipart', 'DomainNet_infograph', 'DomainNet_painting', 'DomainNet_quickdraw', 'DomainNet_real', 'DomainNet_sketch']:
            gpt_descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/domainnet.json'), classnames, dataname)
            descriptions = load_gpt_descriptions(os.path.join(root, 'attributes/domainnet.json'), classnames, dataname)
