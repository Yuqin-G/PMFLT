...
def load_gpt_descriptions(json_file, classnames, dataname):
    ...
    for i, (k, v) in enumerate(filtered_descriptions.items()):
        ...
        if dataname == 'Caltech101' or dataname == 'caltech101':
            prefix = "a photo of a"
            suffix = ""
        if dataname == 'OxfordFlowers' or dataname == 'oxford_flowers':
            prefix = "a photo of a"
            suffix = "a type of flower, "
        if dataname == 'FGVCAircraft' or dataname == 'fgvc_aircraft':
            prefix = "a photo of a"
            suffix = "a type of aircraft, "
        if dataname == 'UCF101' or dataname == 'ucf101':
            prefix = "a photo of a person doing"
            suffix = ""
        if dataname == 'OxfordPets' or dataname == 'oxford_pets':
            prefix = "a photo of a"
            suffix = "a type of pet, "
        if dataname == 'Food101' or dataname == 'food101':
            prefix = "a photo of a"
            suffix = "a type of food, "
        if dataname == 'DescribableTextures' or dataname == 'dtd':
            prefix = "a photo of a"
            suffix = "a type of texture, "
        if dataname =='StanfordCars' or dataname == 'stanford_cars':
            prefix = "a photo of a"
            suffix = ""
